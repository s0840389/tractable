...
% (11) capital price
 ...
 LHS(11)=qk;
 RHS(11)=1+p.tau*(Invminus/Kminus-p.delta);
...
% (13) expected capital price
 ...
 LHS(13)=Eqkminus;
 RHS(13)=(1/(1+ra))*(pk-p.tau/2*(Inv/K-p.delta)^2+p.tau*(Inv/K-p.delta)*Inv/K+(1-p.delta)*Eqk);
 
 % (14) capital lom
 
 LHS(14)=K;
 RHS(14)=(1-p.delta)*Kminus+Invminus-p.tau/2*(Invminus/Kminus-p.delta)^2*Kminus;
...
% (19) resource constraint
 ...
 LHS(19)=Yminus;
 RHS(19)=Cminus + Gminus + Invminus + p.tau/2*(Invminus/Kminus-p.delta)^2*Kminus+0*chi;
 
% (20) Interest rate

 LHS(20)=int;
 RHS(20)=p.rhoint*intminus+ (1-p.rhoint)*(p.phipi*(pitminus-p.pitstar)+p.intstar) + eintminus;
...
% (21) wage inflation
 ...
 LHS(26)=Cminus;
 RHS(26)=p.shrCap*Ccapminus+(1-p.shrCap)*Cwminus;

 % (27) capatalsit consumption
 
 LHS(27)=Ccapminus;
 RHS(27)=-dminus/p.shrCap;
